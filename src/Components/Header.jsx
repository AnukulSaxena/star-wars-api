...
import { setPlanets, setPlanetUrl } from "../store/homeSlice";
...
const Header = () => {
  ...
  return (
    <div className="w-full flex justify-between px-10 bg-inherit h-14 fixed top-0 shadow-2xl z-50">
      ...
      <div className="w-fit text-neutral-200 flex py-2 justify-center items-center gap-5 h-full">
        <div
          onClick={() => {
            navigate("/");
          }}
          className={`w-fit flex justify-center items-center  h-full ${!pageNum ? "border-b dark:border-neutral-200" : ""}`}
        >
          Planets
        </div>
        <div
